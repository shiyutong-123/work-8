...
class SelectorList(list[_SelectorType]):
    ...
    def xpath(
        ...
        **kwargs: Any,
        ...
        """
        _sanitize_query(xpath)
        return self.__class__(
            flatten([x.xpath(xpath, namespaces=namespaces, **kwargs) for x in self])
        )

    def css(self, query: dict) -> SelectorList[_SelectorType]:
        """
...
class Selector:
    ...
    def css(self, query: dict) -> SelectorList[Self]:
        """
        ...
        return self.xpath({'raw': self._css2xpath(query)})

    def _css2xpath(self, query: dict) -> str:
